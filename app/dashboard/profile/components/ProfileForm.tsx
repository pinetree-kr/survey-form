...
export default function ProfileForm({ profile, updateProfile }: ProfileFormProps) {
    ...
    const handleSubmit = async (formData: FormData) => {
        ...
        try {
            ...
            setMessage({
                type: 'error',
                text: error instanceof Error ? error.message : '프로필 업데이트에 실패했습니다.'
            })
        } finally {
            setIsLoading(false)
        }
        ...
        <form action={handleSubmit} className="space-y-6">
            ...
            <div>
                ...
                <div className="flex items-center space-x-2">
                    <span className={`inline-flex px-3 py-2 text-sm font-semibold rounded-md ${profile.role === 'admin'
                        ? 'bg-red-100 text-red-800'
                        : profile.role === 'moderator'
                            ? 'bg-yellow-100 text-yellow-800'
                            : 'bg-green-100 text-green-800'
                        }`}>
                        {profile.role === 'admin' ? '관리자' : profile.role === 'moderator' ? '모더레이터' : '사용자'}
                    </span>
                    <span className="text-sm text-gray-500">
                        {
                            profile.role === 'admin' ? '(본인의 관리자 권한은 변경할 수 없습니다)' : '(권한은 관리자만 변경할 수 있습니다)'
                        }
                    </span>
                </div>
            </div>
